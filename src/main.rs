...
fn run_ci_test() {
    ...
    let kdu = factory.create_kdu(&originator_keypair, originator_fqei, "CITest".to_string(), &payload);
    ...
    let signature = ed25519_dalek::Signature::from_bytes(kdu.originator_signature.as_slice().try_into().unwrap());
...
fn run_processor() {
    ...
    let response_payload = TestPayload { action: "sovereign.pipe.response".to_string(), status: "ok".to_string() };
    let response_kdu = factory.create_kdu(&ponger_keypair, ponger_fqei, "PongResponse".to_string(), &bincode::serialize(&response_payload).unwrap());
...
fn run_client(github_token: &str) {
    ...
    let response = ureq::post(GITHUB_API_URL)
        .set("Accept", "application/vnd.github.v3+json")
        // Use the modern "Bearer" format for the token.
