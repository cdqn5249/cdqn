...
5.  **Asynchronous Performance:** Built on a **non-blocking runtime** that maximizes concurrency for all operations, inheriting this model into **`cdqnLang`**.
...
9.  **Self-Correcting Growth:** Learning occurs by analyzing failed paths (Dpaths) to generate new, verified **Meta-Axioms**, ensuring the system's logic evolves based on verifiable contradiction.
...
| :--- | :--- | :--- |
| **CDU** | **Causal Data Unit.** The immutable, content-addressed container holding data, metadata (HLC, Polarity, Author ID, Reputation), and payload. | Immutability |
| **HLC** | **Hybrid Logical Clock.** Ensures strict, distributed temporal ordering. | K.HLC Module |
| **K-Module** | **Kernel Module.** Hosts the pure, immutable logic: the **Four Core Primitives** ($\mathcal{V}, \mathcal{C}, \mathcal{I}, \mathcal{L}$). | Root of Logic |
| **C-Module** | **Core Runtime.** Manages asynchronous execution, I/O, Gas Metering, and enforces the **Harm Guardrail**. | Execution & Security |
| **Axiom CDU** | A special CDU defining a rule (e.g., inference, polarity assignment, world law). These are the system's explicit knowledge base. | Symbolic Logic |
| **Polarity** | A discrete value ($\{-1, 0, 1\}$) indicating immediate semantic valence, used for fast guardrail checks. | CDU Metadata |
...
| **Prime Elements** | Fixed, discrete numbers ($P_3, P_5, \dots$) that anchor the semantic space and define levels of factual certainty. | Semantic Anchors |
...
| **Reputation Score** | A dynamic score maintained locally in the C-Module, earned through successful CDU validation and decaying over time. | Local Trust Metric |

***

This final manifesto consolidates the entire validated design, emphasizing the system's commitment to **verifiable causality, symbolic reasoning, and decentralized, earned trust.**
