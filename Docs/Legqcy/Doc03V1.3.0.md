...
## **3. The Kernel Data Unit (KDU) Type System**

The KDU is the universal data structure. The ecosystem is built upon a small, elegant, and non-overlapping set of 12 official `kdu_type`s, organized into four logical families.

### **a. The KDU Schema in `cdqnLang`**
This is the definitive structure for every KDU in the ecosystem.
...
struct KDU
  ...
  string: kdu_type          // The type from the list below
...
### **b. The 12 Official KDU Types**

*   **Core Data & Knowledge:** `Generic`, `Procedural`, `Workflow`, `WorldLaw`.
*   **System Configuration & Definition:** `Configuration`, `UnisphereIndex`.
*   **Economic & Legal Instruments:** `Contract`, `ProofOfEvent`, `NotarizedMintingOrder`.
*   **Learning & Governance Loop:** `Feedback`, `VulnerabilityReport`, `Proposal`.

### **c. Consolidated and Deprecated Types**
To simplify the ecosystem, several concepts are now implemented as patterns using the types above, rather than as unique KDU types.
*   **`ModuleDefinition`:** Is now a `Configuration` KDU with `config_type: "Module"`.
*   **`AssetPointer`:** Is now a `Generic` KDU containing a well-defined `struct`.
*   **`SwarmJoinRequest`, `RewardClaim`, etc.:** Are now `Generic` KDUs with a specific `action` field in their payload.
  ...
  behavior KDU: message → (state, list[KDU])
    ...
    if message.action = "metrics.low_confidence_cluster_detected"
      ...
      KDU: request ← KDU.new
        ...
        payload: { action: "summarize_cluster", ids: message.payload.cluster_ids }
      
      return state, [request]
    
    → message.action = "analyst.topic_summary_received"
      // 3. It now has the "why". It creates a formal proposal to design a solution.
      Proposal: new_sphere_proposal ← Proposal.new
        proposal_type: "CustomSphere"
        proposal_data: {
          domain_of_incompetence: message.payload.summary
        }
      
      KDU: proposal_kdu ← KDU.new
        kdu_type: "Proposal"
        target: "C.Curation.GovernanceBot" // The entity that manages the next steps
        payload: new_sphere_proposal
        
      return state, [proposal_kdu]
    /if
  /behavior
/bot
```
