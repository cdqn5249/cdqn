...
| **K.CryptoCore** | **Performs all cryptographic operations** (hashing, signing, signature verification). | Called by $\mathcal{C}$ to ensure integrity. |
...
| :--- | :--- | :--- | :--- |
| **`id`** | `[u8; 32]` (SHA-256 Hash) | 32 Bytes | **Content Address.** |
