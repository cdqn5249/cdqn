...
### 2.2 The Law of Erasure
*   **Constraint:** To ensure the arrow only points forward, $S_t$ must be destroyed upon the creation of $S_{t+1}$.
*   **Implementation:** As defined in `02f-SECURITY`, this requires **Secure Zeroing** of memory (Hardware Enforced in Tier 2, Best Effort in Tier 1).
...
*   **Mechanism:** The vector updates itself to align with the **Majority Rule** (Bundling from `02a`) of its weighted connections.
...
*   **$m_0$:** The Intrinsic Mass (Rank) from `02a`.
*   **Hypothesis:** By linking Inertia to **Information Density** and **Connectivity**, the system should naturally resist "Gaslighting" (low-mass attacks on high-mass truths) without requiring hard-coded guardrails.
