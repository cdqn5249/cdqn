...
*   **Layer 3: PHYSICS (The Arrow).**
    *   **Tropical Geometry:** History is a cryptographic hash chain anchored to the Genesis Stream. Time is monotonic ($S_{t+1} = \max(S_t, S_{new})$). It is mathematically impossible to "undo" an event.
...
7.  **Martín-Olalla, J.M.** (2025). *"Thermal stability."* (Physics of Memory).
