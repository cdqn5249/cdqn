# 03e-CHEMISTRY: The Interaction API

*   **File:** `docs/research/03e-CHEMISTRY.md`
*   **Repository:** [https://github.com/cdqn5249/cdqn](https://github.com/cdqn5249/cdqn)
*   **Author:** Christophe Duy Quang Nguyen (System Ronin)
*   **Context:** Layer 4 Specification (Valency, Bonding, Linear Logic & The Quantale Economy)
*   **Date:** December 12, 2025
*   **Status:** `v1.2` (The Metabolist Standard)

> **The Engine of Transaction.**
> *Layer 3 defined the object. Layer 4 defines the relationship. This API implements the **Quantale Economy**: a system where every interaction is a chemical reaction that consumes resources (Energy) to produce results. It rejects "CPU-only" value generation, implementing a **Bio-Mimetic Metabolism** where energy comes from Work (Mechanical), Attention (Symbiotic), and Trade (Commercial). It enforces **Mass-based Valency** to prevent structural spam and includes a **Recycling Loop** to recover energy from broken structures.*
...
2.  **Diverse Metabolism:** Energy is generated via three pathways: Mechanical (CPU), Symbiotic (Human), and Commercial (Trade).
3.  **Valency Constraints:** **$Slots \propto Mass$**. Heavy concepts support more connections.
4.  **Endothermic Physics:** Creating and Breaking bonds consumes Energy.
...
### 2.3 The Reaction Context
The environment required for a transaction to occur.

```rust
pub struct ReactionContext {
    pub energy_budget: EnergyToken, // The fuel for the reaction
    pub catalyst_id: Option<u64>,   // Optional: Code/Rule facilitating the reaction
}
```
...
### 3.3 Commercial Mint (Trade)
*   **Trigger:** A successful `LVM_BARTER` (Layer 8 protocol).
*   **Mechanism:** When Node A sends data to Node B, and Node B accepts it (Rank Increase), Node A receives a "Trade Surplus."

```rust
/// Mint energy from successful trade.
/// @param receipt: Proof that another node accepted our data.
fn lvm_mint_commercial(receipt: TradeReceipt) -> EnergyToken;
```

### 3.4 Recycling (The Loop)
...
The correctness of this API will be validated by `docs/validation/03e_chemistry.check`.
