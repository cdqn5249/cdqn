...
This document is a **manifesto, not a specification**: it declares principles, laws, and boundaries. The constructive detail — digit seeds, binary ABI, composite topologies — lives in the specifications that derive from it, and may evolve in later versions without touching this axiom, provided the laws below hold.
...
> _"Can a computing number system abstract arbitrary state and reasoning?"_
...
We do not claim mathematical proofs. The Qn universe is **autonomous** and bound to finitism (§I.2): it has **not defined infinity**, and the proofs of IRL mathematics — which lean on completed infinities and limits — cannot be derived within it. Qn is therefore **not required to integrate with IRL mathematics**: it carries its own axioms and borrows none, least of all completed infinity. Instead we commit to a three-step method:

1. Define axioms and primitives precisely.
2. Derive consequences logically and consistently.
3. Verify by computation: the QnLang compiler shall execute our constructions and confirm, within bounded zoom windows, that they agree with the known IRL results of mathematics, semantics, and physics that they are intended to model.

Verification is a **one-way consistency check against an external oracle**: it tests our constructions; it does not adopt IRL axioms. The design of limits toward any target — including an operational tending-toward-infinity — is reserved to the Quang Maths (Qm) specification, a downstream construction that remains to be built. All hardness assumptions are labeled **conjectures**. All derived identities are marked **to be verified by QnLang**.
...
Four mathematical constraints maintain deterministic evaluation, verifiability, and lawful scope.

### V.1 Finitism & Bounded Model Checking

Qn state spaces are strictly bounded and discrete; verification uses deterministic model checking and SMT over finite lattices, avoiding floating-point drift.

### V.2 Total Functionality (Turing Incompleteness)

`QnIR` is total. Every instruction carries a derivable upper bound on time and space prior to execution. We derive this by construction; QnLang shall enforce and verify it.

### V.3 Fractal Homoiconicity & Qexpr

Because the Triad is the universal existence condition, all entities (data atoms, instructions, agent states, routing nodes) share the identical Qn contract and are expressed as `Qexpr`.

### V.4 Scope Separation & Capability Bounding (the DCC Instrument)

> **Governed by:** `[M]` · `[Mod]`

Every Qn inherently spans the concerns of both twin domains — the mathematical (`Qm`) and the semantic (`Qs`). Each property of a Qn belongs to **its own concern** and operates within **its own scope**. The DCC profile `⟨ D_dep, C_con, K_cap ⟩` is the instrument that bounds each property's scope and capability: dependencies ground it, capabilities permit it, and constraints — negative capabilities — forbid it. No property may act beyond the scope its DCC declares. The concrete per-atom DCC assignments are constructed in the seed layer ([qn-base.md](qn-base.md)).
...
**The stack is open, not a closed ladder.** A new layer — including an intermediate layer between two existing ones — may be introduced whenever it satisfies the four criteria above. The current layer map is a construction-in-progress, maintained in the documentation portal, and may be revised as the derivation matures; no enumeration of layers is frozen by this manifesto.
...
`D` operates in two fundamental regimes:
...
A construction may additionally yield **derived dimensional markers** — for example spatial or solvability dimensions — that are consequences of that construction rather than members of the Hurwitz set. Such markers are governed by the Constructibility Principle (§VI), are specified in the seed layer ([qn-base.md](qn-base.md)), and must never be read as normed-division-algebra dimensions. The two regimes above remain the law; derived markers are their downstream consequences.
